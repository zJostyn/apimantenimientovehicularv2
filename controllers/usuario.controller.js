...
async function verCorreosRegistrados(req, res) {
    ...
    try {
        ...
        if (result.rowCount >= 1) {
            ...
            res.status(200).json('Correo no registrado!')
        }
    } catch (err) {
        res.status(500).json({ error: 'Error en el servidor!' });
    }
...
async function verUsuarioYaRegistrado(req, res) {
    const { nombredeusuario } = req.body;
    const query = 'SELECT * FROM usuario WHERE nombredeusuario = $1';
    const values = [nombredeusuario];
    try {
        const client = await pool.connect();
        const result = await client.query(query, values);
        client.release();
        if (result.rowCount >= 1) {
            res.status(400).json('Ya existe un usuario igual a este!')
        } else {
...
module.exports = {
    ...
    habilitarUsuario,
    bloquearUsuario,
    verCorreosRegistrados,
    verUsuarioYaRegistrado
};